//CHECKOUT BEFORE FESTIVAL PLACEOREDR
...
function CheckOut() {
  ...
  const [addressInput, setFullAddressInput] = useState("")
  ...
  const ALLOWED_CITIES = ["Hubli", "Dharwad"]

  const isAddressInsideCity = (addressText, city) => {
    if (!addressText || !city) return false
  ...
  const getAddressByLatLng = async (lat, lng) => {
    try {
      ...
      dispatch(setFullAddress(result?.data?.results[0].address_line2))
    } catch (error) {
      console.log(error)
    }
  }
  ...
  cartItems.map(i => ({
    ...
    shop: i.shop
      ...
      // Check if address is inside selected city
      if (!isAddressInsideCity(addressInput, currentCity)) {
        setCityValidationError(`Delivery address must be inside ${currentCity}.`)
        alert(`Delivery address must be inside ${currentCity}.`)
        return
      }
      ...
      // Validate cartItems have proper shopId
...
cartItems: cartItems.map(item => ({
  food: item.food,        // ✅ IMPORTANT
  _id: item._id,          // fallback
  quantity: item.quantity,
  price: item.price,
  shop: item.shop._id     // send only ObjectId
})),
...
const openRazorpayWindow=(orderId,razorOrder)=>{
 ...
 if (!window.Razorpay) {
    alert("Razorpay SDK not loaded. Please check your internet or the script in index.html");
    return;
  }
      ...
      <div className='w-full max-w-[900px] bg-white rounded-2xl shadow-xl p-6 space-y-6'>
        ...
        <section>
          ...
          <div className='mb-3 pb-3 border-b'>
            <p className='text-sm font-medium text-gray-700 mb-2'>Selected City: <span className='text-[#ff4d2d] font-bold'>{currentCity || "Not selected"}</span></p>
          </div>
          <div className='flex gap-2 mb-3'>
            <input type="text" className={`flex-1 border rounded-lg p-2 text-sm focus:outline-none focus:ring-2 ${cityValidationError ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-[#ff4d2d]'}`} placeholder='Enter Your Delivery Address..' value={addressInput} onChange={(e) => {
              setFullAddressInput(e.target.value)
              if (cityValidationError && isAddressInsideCity(e.target.value, currentCity)) {
                setCityValidationError("")
              }
            }} />
